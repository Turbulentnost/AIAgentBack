...
def test_attach_email_files_still_attaches_msg_when_file_fetch_fails():
    integration = StubIntegrationService()
    email = _email_with_attachment(content=None)
    vault = MagicMock()
...
def test_erp_email_upload_marker_names_includes_legacy_and_msg():
    ...
    assert ERP_LEGACY_EMAIL_FILENAME in names
    assert "Входящее_письмо.msg" in names


def test_collect_erp_upload_files_converts_to_msg():
