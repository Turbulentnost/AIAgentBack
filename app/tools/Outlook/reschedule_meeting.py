...
Примеры:
  ...
  python -m app.tools.Outlook.reschedule_meeting --id "AQMkAD..." --changekey "DwAA..." --new-start "2026-06-10 15:00" --dry-run
  python -m app.tools.Outlook.reschedule_meeting --id "AQMkAD..." --new-start "2026-06-10 15:00" --yes
  python -m app.tools.Outlook.reschedule_meeting --subject "Тестовое" --start "2026-06-09 10:00" --new-start "2026-06-09 14:00" --yes
  python -m app.tools.Outlook.reschedule_meeting --id "AQMkAD..." --new-start "2026-06-10 15:00" --duration 30 --message "Перенос" --yes
"""
...
def reschedule_meeting_item(
    ...
    *,
    ...
    new_start = to_local(new_start, config)
    new_end = to_local(new_end, config)
    if new_end <= new_start:
        raise ValueError("Конец совещания должен быть позже начала.")

    item.start = to_ews(new_start, config)
    item.end = to_ews(new_end, config)
    ...
    if message.strip():
        note = message.strip()
        existing = str(item.body or "").strip()
        item.body = HTMLBody(f"{existing}\n\n<p>{note}</p>" if existing else note)
    ...
    item.save(update_fields=update_fields, send_meeting_invitations=SEND_ONLY_TO_ALL)
