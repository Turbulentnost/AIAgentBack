"""LLM prompts for accountant_agent — bodies live in system.md / user.md."""
...
from app.agents.accountant_agent.decisions import AccountantAssessment
...
def extract_untrusted_memo(request: AccountantAgentRequest) -> str:
    parts: list[str] = []
...
def build_case_json(
    ...
    blob = {
        ...
        "fully_approved": request.case_context.fully_approved,
    }
...
def build_task_hint(
    ...
    pr = assessment.payment_request_id or "?"
    return (
        f"Payment status {pr}: {assessment.payment_status}, "
        f"delay {assessment.delay_days} days."
    )
