"""LLM prompts for chief_accountant_agent — bodies live in system.md / user.md."""
...
from app.agents.chief_accountant_agent.decisions import ChiefAssessment
...
def extract_untrusted_memo(request: ChiefAccountantAgentRequest) -> str:
    parts: list[str] = []
...
def build_task_hint(
    ...
    pr = assessment.payment_request_id or assessment.registry_id or "?"
    return f"Accounting check for payment {pr}, issues={len(assessment.issues)}."
