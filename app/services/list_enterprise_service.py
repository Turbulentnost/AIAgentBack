...
Подключение к 1С через OData вынесено в app.integrations.onec_odata.
"""
...
def is_marked_or_archived(row: dict) -> bool:
    for field in ("DeletionMark", "Недействителен", "ВАрхиве"):
        if row.get(field) is True:
            return True
    status = normalize_text(str(row.get("Статус") or row.get("State") or ""))
    name = normalize_text(row.get("Description"))
    if "ликвид" in status or "ликвид" in name:
        return True
    return False
...
def load_hierarchy(
    ...
    for row in fetch_all(session, url):
        key = row.get("Ref_Key")
        if not key or is_marked_or_archived(row):
            continue
        by_key[key] = row
...
def build_enterprise_departments(session: requests.Session) -> list[dict]:
    """Return active 1C enterprise departments, excluding deleted/archived/liquidated nodes."""
    ...
    for key, row in structure.items():
        ...
        rows.append(
            {
                "external_id": key,
                "parent_external_id": None if is_empty_key(parent_key) else parent_key,
                "name": name,
                "path": structure_path(key, structure),
            }
        )
...
def main() -> None:
    ...
    try:
        output_path = save_report(build_report(create_session(), root_name), root_name)
