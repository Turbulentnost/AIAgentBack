...
from app.models.enums import MeetingRegistryStage, ScheduledMeetingStatus
...
from app.services.scheduled_meeting_outlook import (
    ...
    _attendee_emails,
    cancel_scheduled_meeting_in_outlook,
...
from app.services.scheduled_meeting_person import (
    ...
    resolve_users_for_position_ids,
)
from app.services.scheduled_meeting_outlook_participants import sync_series_participants_in_outlook
from app.services.scheduled_meeting_outlook_update import (
    update_series_end_date_in_outlook,
    update_series_recurrence_in_outlook,
)
from app.services.scheduled_meeting_recurrence import (
    apply_recurrence_payload_to_meeting,
...
from app.services.scheduled_meeting_roles import (
    ...
    protected_participant_user_ids,
)
...
class ScheduledMeetingService:
    ...
    async def list_employee_options(
        ...
        *,
        ...
        options = await list_employee_options(self.db, search=search, limit=limit)
        return [
            ScheduledMeetingEmployeeOptionRead(
                id=item.id,
                fio=item.fio,
    ...
    async def resolve_positions(
        ...
        results = await resolve_users_for_position_ids(self.db, position_ids)
        return ScheduledMeetingPositionResolveRead(
            items=[
                ScheduledMeetingPositionResolveItemRead(
                    position_id=item.position_id,
                    position_name=item.position_name,
                    ...
                    employee=(
                        ScheduledMeetingEmployeeOptionRead(
                            ...
                            position_name=item.employee.position_name,
                        )
    ...
    async def list(self) -> list[ScheduledMeetingRead]:
        result = await self.db.execute(
            ...
            .options(
                ...
                selectinload(ScheduledMeeting.manager_user),
                selectinload(ScheduledMeeting.responsible_user),
                selectinload(ScheduledMeeting.participants).selectinload(
                    ScheduledMeetingParticipant.position
                ),
                selectinload(ScheduledMeeting.participants).selectinload(
                    ScheduledMeetingParticipant.user
                ),
            )
    ...
    async def _resolve_participant_create(
        self,
        participant: ScheduledMeetingParticipantCreate,
    ) -> ScheduledMeetingParticipantCreate:
        if participant.user_id is None and not (
            participant.person_fio and participant.person_email
        ):
            raise ScheduledMeetingServiceError(
                "У каждого участника серии должны быть указаны ФИО и e-mail",
                status_code=400,
            )
        try:
        ...
        return ScheduledMeetingParticipantCreate(
            user_id=person.user_id,
            person_fio=person.fio,
            person_email=person.email,
            position_id=participant.position_id or person.position_id,
            sort_order=participant.sort_order,
            is_required=participant.is_required,
        )

    async def _resolve_role_person(
        self,
        user_id: uuid.UUID,
        *,
        role_label: str,
    ...
    async def _require_position_id(
        self,
        person: ResolvedPerson,
        *,
        role_label: str,
        explicit_position_id: uuid.UUID | None = None,
    ) -> uuid.UUID:
        position_id = explicit_position_id or person.position_id
        if position_id is None:
            position_id = await resolve_position_id_for_person(self.db, person)
        if position_id is None:
            raise ScheduledMeetingServiceError(
                f"Не удалось определить должность для {role_label} «{person.fio}»",
                status_code=400,
            )
        await self._ensure_positions_exist([position_id])
        return position_id
    ...
    async def create(self, payload: ScheduledMeetingCreate) -> ScheduledMeetingRead:
        ...
        manager = await self._resolve_role_person(
            payload.manager_user_id,
            role_label="руководитель",
            person_fio=payload.manager_person_fio,
            person_email=payload.manager_person_email,
        )
        responsible = await self._resolve_role_person(
            payload.responsible_user_id,
            role_label="ответственный",
            person_fio=payload.responsible_person_fio,
            person_email=payload.responsible_person_email,
        )
        manager_position_id = await self._require_position_id(
            manager,
            role_label="руководителя",
            explicit_position_id=payload.manager_position_id,
        )
        responsible_position_id = await self._require_position_id(
            responsible,
            role_label="ответственного",
            explicit_position_id=payload.responsible_position_id,
        )

        resolved_participants = [
            await self._resolve_participant_create(participant)
        ...
        position_ids = [
            item.position_id for item in merged_participants if item.position_id is not None
        ]
    ...
    async def cancel(
        self,
        meeting_id: uuid.UUID,
        payload: ScheduledMeetingCancelRequest,
        *,
        current_user: User,
    ) -> ScheduledMeetingCancelRead:
        meeting = await self._load_meeting(meeting_id)
        ...
        if meeting.status == ScheduledMeetingStatus.ARCHIVE:
            return ScheduledMeetingCancelRead(
        ...
        if meeting.status != ScheduledMeetingStatus.PLANNED:
            raise ScheduledMeetingServiceError(
                "Отменить можно только распланированную серию",
                status_code=409,
            )
        ...
        if meeting.outlook_series_id:
            cancel_payload, outlook_cancelled, outlook_warning = await self._cancel_outlook_series(
                meeting,
                message,
            )
        else:
            outlook_warning = (
                "Серия не связана с календарём Outlook. Запись будет перенесена в архив."
            )
        ...
        if entry is not None:
            if entry.stage == MeetingRegistryStage.CANCELLED:
                pass
            elif registry_cancel_allowed(entry.stage):
                await registry.mark_cancelled(
                    memo_ref_key=entry.memo_ref_key,
                    cancelled_by=current_user,
                    message=message or "Серия совещаний отменена",
                    cancel_payload=cancel_payload,
                    outlook_cancelled=outlook_cancelled,
                )
            else:
                registry_warning = (
                    "Карточка реестра не обновлена: совещание уже проведено или протокол закрыт"
                )

        meeting.status = ScheduledMeetingStatus.ARCHIVE
        stored_payload = dict(meeting.payload or {})
        stored_payload["cancelled_at"] = datetime.now(
            ZoneInfo(settings.OUTLOOK_TIMEZONE)
        ).isoformat()
        if cancel_payload:
            stored_payload["cancel_payload"] = cancel_payload
    ...
    async def update(
        ...
        if change_set.unsupported_fields:
            ...
            raise ScheduledMeetingServiceError(
                "Пока поддерживается изменение срока серии, периодичности, комментария и состава "
                "участников (добавление и удаление). "
        ...
        if (
            ...
            and not change_set.recurrence_changed
        ...
        if change_set.manager_changed and change_set.new_manager_user_id is not None:
            manager = await self._resolve_role_person(
                change_set.new_manager_user_id,
                role_label="руководитель",
            )
            meeting.manager_user_id = manager.user_id
            meeting.manager_position_id = await self._require_position_id(
                manager,
                role_label="руководителя",
                explicit_position_id=change_set.new_manager_position_id,
            )
            changes.append("manager")
            role_participants_need_sync = True
        if change_set.responsible_changed and change_set.new_responsible_user_id is not None:
            responsible = await self._resolve_role_person(
                change_set.new_responsible_user_id,
                role_label="ответственный",
            )
            meeting.responsible_user_id = responsible.user_id
            meeting.responsible_position_id = await self._require_position_id(
                responsible,
                role_label="ответственного",
                explicit_position_id=change_set.new_responsible_position_id,
            )
        ...
        if role_participants_need_sync:
                    ...
                    for participant in (
                        ...
                        if change_set.new_participants
                            ...
                            for participant in sorted(
                                meeting.participants, key=lambda item: item.sort_order
                            )
            ...
            new_ids = [item.user_id for item in merged_participants if item.user_id is not None]
            participant_add_ids = tuple(user_id for user_id in new_ids if user_id not in set(current_ids))
            participant_remove_ids = tuple(
                user_id for user_id in current_ids if user_id not in set(new_ids)
            )
            participants_added_names, participants_removed_names = await self._apply_participant_changes(
                ...
                added_user_ids=participant_add_ids,
                removed_user_ids=participant_remove_ids,
            )
        ...
        if change_set.participants_changed and not role_participants_need_sync:
            resolved_participants = [
                await self._resolve_participant_create(participant)
                for participant in change_set.new_participants
            ]
            new_ids = {item.user_id for item in resolved_participants if item.user_id is not None}
            if not protected_participant_user_ids(meeting).issubset(new_ids):
                raise ScheduledMeetingServiceError(
        ...
        if (
            ...
            try:
                add_emails = (
                    await self._resolve_emails_for_user_ids(participant_add_ids)
                    if participant_add_ids
                    else []
                )
                remove_emails = (
                    await self._resolve_emails_for_user_ids(participant_remove_ids)
        ...
        if change_set.recurrence_changed and change_set.new_recurrence is not None:
            apply_recurrence_payload_to_meeting(meeting, change_set.new_recurrence)
            ...
            if meeting.outlook_series_id and meeting.status == ScheduledMeetingStatus.PLANNED:
                try:
                    outlook_result = await update_series_recurrence_in_outlook(self.db, meeting)
                except ScheduledMeetingOutlookError as exc:
                    raise ScheduledMeetingServiceError(
                        str(exc),
                        status_code=exc.status_code,
                ...
                if isinstance(action, str) and action:
                    outlook_actions.append(action)
                meeting.outlook_changekey = (
                    outlook_result.get("outlook_changekey") or meeting.outlook_changekey
                )
                meeting.outlook_meeting_url = (
                    outlook_result.get("outlook_meeting_url") or meeting.outlook_meeting_url
                )
        elif change_set.series_end_changed:
            if meeting.outlook_series_id and meeting.status == ScheduledMeetingStatus.PLANNED:
                try:
        ...
        if (
            change_set.series_end_changed
            or change_set.recurrence_changed
            or change_set.participants_changed
        ) and (meeting.status == ScheduledMeetingStatus.PLANNED):
    ...
    async def _participant_names_for_user_ids(
        self,
        ...
        names_by_id = {
            user.id: (user.full_name or user.email or str(user.id)).strip()
            for user in result.scalars().all()
        }
        return [names_by_id[user_id] for user_id in user_ids if user_id in names_by_id]

    async def _resolve_emails_for_user_ids(
        self,
        user_ids: tuple[uuid.UUID, ...],
    ) -> list[str]:
        if not user_ids:
            return []
        result = await self.db.execute(
            select(ScheduledMeetingParticipant.person_email, ScheduledMeetingParticipant.user_id)
            .where(ScheduledMeetingParticipant.user_id.in_(user_ids))
        )
        emails_by_user: dict[uuid.UUID, str] = {}
        for email, user_id in result.all():
            if user_id is not None and email:
                emails_by_user[user_id] = email.strip()
        emails: list[str] = []
        for user_id in user_ids:
            stored = emails_by_user.get(user_id)
            if stored:
            ...
            if user is None or not user.email:
                raise ScheduledMeetingServiceError(
                    f"Не удалось определить e-mail участника: {user_id}",
                    status_code=400,
                )
    ...
    async def _apply_participant_changes(
        ...
        *,
        ...
        added_user_ids: tuple[uuid.UUID, ...],
        removed_user_ids: tuple[uuid.UUID, ...],
    ) -> tuple[list[str], list[str]]:
        position_ids = [item.position_id for item in participants if item.position_id is not None]
        await self._ensure_positions_exist(position_ids)
        added_set = set(added_user_ids)
        removed_set = set(removed_user_ids)
        existing_by_user = {
            item.user_id: item for item in meeting.participants if item.user_id is not None
        }
        ...
        for index, participant in enumerate(participants):
            ...
            if existing is not None:
                ...
                existing.person_fio = participant.person_fio
                existing.person_email = participant.person_email
                existing.position_id = participant.position_id

        added_names = await self._participant_names_for_user_ids(added_user_ids)
        removed_names = await self._participant_names_for_user_ids(removed_user_ids)
    ...
    def _outlook_cancel_not_found(exc: BaseException) -> bool:
        text = str(exc).lower()
        return "не найден" in text or "not found" in text

    async def _cancel_outlook_series(
        self,
        meeting: ScheduledMeeting,
        message: str,
    ) -> tuple[dict | None, bool, str | None]:
        try:
            cancel_payload = await cancel_scheduled_meeting_in_outlook(meeting, message=message)
            ...
            if self._outlook_cancel_not_found(exc):
                warning = (
                    "Серия не найдена в Outlook/Exchange. "
                    "Статус в графике будет обновлён на «Архив»."
                )
                return {"status": "not_found", "error": str(exc)}, False, warning
            raise ScheduledMeetingServiceError(str(exc), status_code=exc.status_code) from exc
            ...
            if self._outlook_cancel_not_found(exc):
                ...
                return {"status": "not_found", "error": str(exc)}, False, warning
            raise ScheduledMeetingServiceError(
                f"Не удалось отменить серию в Outlook/Exchange: {exc}"
            ) from exc
    ...
    def to_read(self, meeting: ScheduledMeeting) -> ScheduledMeetingRead:
        participants = [
            ScheduledMeetingParticipantRead(
                ...
                user_id=participant.user_id,
                person_fio=(
                    participant.person_fio
                    or (
                        participant.user.full_name.strip()
                        if participant.user is not None and participant.user.full_name
                        else None
                    )
                ),
                person_email=participant.person_email,
        ...
        return ScheduledMeetingRead(
            ...
            manager_user_id=meeting.manager_user_id,
            manager_user_fio=(
                meeting.manager_user.full_name if meeting.manager_user is not None else None
            ),
            ...
            responsible_user_fio=(
                meeting.responsible_user.full_name
                if meeting.responsible_user is not None
                else None
            ),
