...
class TaskResult(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    report_bucket: Mapped[str | None] = mapped_column(String(255))
    report_object_name: Mapped[str | None] = mapped_column(String(1024))
    report_url: Mapped[str | None] = mapped_column(String(2048))
    is_current: Mapped[bool] = mapped_column(Boolean, default=True, index=True)
    generated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    metadata_: Mapped[dict | None] = mapped_column("metadata", JSONB)

    # Legacy raw payload kept for compatibility with earlier worker code.
