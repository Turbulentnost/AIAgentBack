...
from sqlalchemy import and_, cast, case, func, literal, or_, select
...
def is_dialog_message(*, status: str = "", payload: dict[str, Any] | None = None) -> bool:
    """True, если письмо относится к категории «Диалог» (не отправлять в 1С ERP)."""
    if (status or "").strip() == "dialog":
        return True
    if not payload:
        return False
    ...
    return bool(dialog) or document_kind == "dialog"
...
def email_eligible_for_erp(
    *,
    ...
    if is_dialog_message(status=status, payload=payload):
        return False
    if payload is None:
...
TURBO_DON_DOMAIN_SUFFIX = "@turbo-don.ru"


def is_turbo_don_email_address(addr: str) -> bool:
    return normalize_email_address(addr).endswith(TURBO_DON_DOMAIN_SUFFIX)


def is_turbo_don_routing_row(*, mailbox: str, payload: dict[str, Any] | None) -> bool:
    """True, если попытка маршрутизации относится к ящику @turbo-don.ru."""
    payload = payload or {}
    routing_raw = payload.get("routing_recipient")
    if routing_raw:
        return is_turbo_don_email_address(str(routing_raw))
    return is_turbo_don_email_address(mailbox)


def turbo_don_routing_sql_filter(mailbox_column, raw_payload_column):
    """SQL: показывать только попытки с routing_recipient @turbo-don.ru (или mailbox @turbo-don.ru)."""
    payload = safe_payload_jsonb(raw_payload_column)
    routing = func.lower(func.coalesce(payload["routing_recipient"].astext, ""))
    routing_ok = and_(routing != "", routing.like(f"%{TURBO_DON_DOMAIN_SUFFIX}"))
    routing_empty = func.coalesce(payload["routing_recipient"].astext, "") == ""
    mailbox_ok = func.lower(mailbox_column).like(f"%{TURBO_DON_DOMAIN_SUFFIX}")
    return or_(routing_ok, and_(routing_empty, mailbox_ok))


def routing_base_message_id_sql(message_id_column):
    """Базовый Message-ID без суффикса #recipient@domain."""
    return func.regexp_replace(message_id_column, r"#[^#]+@.+$", "")
