...
Не сохраняет результаты в PostgreSQL.
...
from agent_pochta.db import repository as db_repository  # noqa: E402
...
def main() -> None:
    original_persist = db_repository.persist_processing_result
    db_repository.persist_processing_result = lambda _state: None
    try:
        app = build_graph()
        for email in SAMPLES:
            result = app.invoke({"email": email})
            _print_result(email, result)
    finally:
        db_repository.persist_processing_result = original_persist


def _print_result(email: EmailMessage, result: dict) -> None:
    print("=" * 70)
    ...
    if routing:
        print(
            f"  Отдел:     {routing.department_id} / {routing.department_name} "
