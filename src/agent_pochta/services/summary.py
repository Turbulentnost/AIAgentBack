...
def summary_ru_system_rules(*, min_sent: int, max_sent: int) -> str:
    ...
    return (
        f"summary_ru ({min_sent}–{max_sent} предложений) — сухая выжимка фактов "
        "для офис-менеджера (третье лицо), не ответ отправителю.\n"
        "Включи только: кто обратился; суть запроса; что сделать внутри; "
        "важные вложения; срок — только если явно указан.\n"
        "Пиши канцелярски и лаконично: без воды, повторов, цитат из письма "
        "и истории переписки.\n"
        "Запрещено: приветствия («Здравствуйте»), благодарности "
        "(«Спасибо за ваше сообщение»), советы адресату, «чем могу помочь».\n"
        "Запрещено в summary_ru: телефоны, email, URL, подписи, должности, "
        "дисклеймеры, рекламные блоки, технический мусор из OCR.\n"
        "Только обычный русский текст: буквы, цифры, пробелы и знаки препинания. "
        "Без *, #, >, \\, markdown/HTML, emoji, управляющих и битых символов."
    )
